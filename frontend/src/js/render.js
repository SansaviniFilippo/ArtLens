...
function logPerfIfNeeded() {
  ...
  // print roughly every 3 seconds
  if (t - perfStats.lastPrint > 3000 && perfStats.samples > 0) {
    const s = perfStats;
    const avg = (x) => (x / s.samples).toFixed(2);
    try {
      console.log(`[Perf] over ${s.samples} samples — crop: ${avg(s.cropMs)} ms, embed: ${avg(s.embedMs)} ms, match: ${avg(s.matchMs)} ms (dbSize=${artworkDB.length}, dim=${dbDim})`);
    } catch (e) {
      // Fallback without DB info if something goes wrong
      console.log(`[Perf] over ${s.samples} samples — crop: ${avg(s.cropMs)} ms, embed: ${avg(s.embedMs)} ms, match: ${avg(s.matchMs)} ms`);
    }
...
export async function benchmarkMatchLoop(iterations = 200, warmup = 20) {
  ...
  for (let i = 0; i < warmup; i++) {
    findBestMatch(randomUnitVec(dbDim));
  }
  const times = [];
